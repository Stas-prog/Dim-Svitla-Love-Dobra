...
type RoomsResp = {
    items: { roomId: string; lastSeen: string }[];
};

async function getRooms(): Promise<RoomsResp["items"]> {
    const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL ?? ""}/api/vision/rooms?limit=100`, {
        cache: "no-store",
    });
    if (!res.ok) return [];
                ...
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                    {rooms.map((r) => (
                        ...
                        >
                            ...
                            <div className="text-[11px] text-slate-400 mt-1">{r.lastSeen}</div>
